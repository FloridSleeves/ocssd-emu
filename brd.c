...
static void brd_make_request(struct request_queue *q, struct bio *bio)
{
    ...
    struct brd_device *brd = bdev->private_data;
    int rw;
    struct bio_vec *bvec;
    sector_t sector;
    int i;
    ...
    debug("w=%lu %lu, sector=%lu\n", bio_op(bio), bio_rw(bio), bio->bi_iter.bi_sector);

    sector = bio->bi_iter.bi_sector;
    if (sector + (bio->bi_iter.bi_size >> SECTOR_SHIFT) > sdk_capacity)
        goto out;
...
out:
    debug("rw=%lu %lu, sector=%lu, err=%d\n", bio_op(bio), bio_rw(bio), bio->bi_iter.bi_sector, err);
    bio_endio(bio, err);
}
...
out:
    debug("rw=%lu, sector=%lu, err=%d\n", bio_rw(bio), bio->bi_iter.bi_sector, err);
    return err;
}
...
repeat:
    ...
    __rq_for_each_bio(bio, req) {
        ...
        if (!(bio->bi_flags & (1 << BIO_CLONED)) && !(bio_op(bio) == REQ_OP_DISCARD) && (bio_op(bio) & 0x1))
            ret = -111;
        else
    ...
    if (unlikely(req->bio_op(bio) == REQ_OP_DISCARD)) {
        type = SSD_ERASE;
    } else if (req->bio_op(bio) & 0x1) {
        type = SSD_WRITE;
    } else {
        type = SSD_READ;
    }
